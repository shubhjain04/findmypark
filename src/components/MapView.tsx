...
const MapView: React.FC<MapViewProps> = ({
  ...
  const handleSearch = (query: string) => {
    ...
    if (mapInstance.current && !apiError) {
      ...
      const filteredLots = mockParkingLots.filter(lot => lot.name.toLowerCase().includes(query.toLowerCase()));
  ...
  return <div className="relative h-full w-full overflow-hidden">
      ...
      width: '100%'
        ...
        {apiError && <div className="absolute inset-0 bg-gray-100 z-0 flex items-center justify-center">
            <div className="bg-white p-4 rounded-lg shadow-lg max-w-md">
              ...
              <div className="relative h-64 w-full bg-gray-200 rounded-lg overflow-hidden">
                {mockParkingLots.map((lot, index) => <div key={lot.id} className="absolute" style={{
              left: `${30 + index * 25}%`,
              top: `${40 + index % 2 * 20}%`
            }} onClick={() => handleSelectLot(lot)}>
                    <ParkingLot lot={lot} onSelect={() => handleSelectLot(lot)} isSelected={selectedLot?.id === lot.id} />
                  </div>)}
              </div>
            </div>
          </div>}
